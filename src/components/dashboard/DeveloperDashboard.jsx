...
import { Activity, GitMerge, ShieldAlert, Zap, Terminal, Sparkles, Mic } from "lucide-react";
...
export default function DeveloperDashboard() {
    ...
    const cacheKey = (name) => `dev_${name}_${CACHE_VERSION}`;

    useEffect(() => {
        async function fetchAll() {
            // Check cache first
            const cachedDora = sessionStorage.getItem(cacheKey('dora'));
            const cachedQuality = sessionStorage.getItem(cacheKey('quality'));
            const cachedRecs = sessionStorage.getItem(cacheKey('recs'));
            const cachedPipes = sessionStorage.getItem(cacheKey('pipelines'));
            ...
            try {
                ...
                if (!doraRes.ok) {
                    throw new Error(`DORA fetch failed: ${doraRes.status} ${await doraRes.text()}`);
                }
                if (!qualityRes.ok) {
                    throw new Error(`Code quality fetch failed: ${qualityRes.status} ${await qualityRes.text()}`);
                }
                if (!recsRes.ok) {
                    throw new Error(`AI recs fetch failed: ${recsRes.status} ${await recsRes.text()}`);
                }
                if (!pipesRes.ok) {
                    throw new Error(`Pipelines fetch failed: ${pipesRes.status} ${await pipesRes.text()}`);
                }
                ...
                // Save to cache
                sessionStorage.setItem(cacheKey('dora'), JSON.stringify(doraData));
                sessionStorage.setItem(cacheKey('pipelines'), JSON.stringify(pipesData));
                sessionStorage.setItem(cacheKey('quality'), JSON.stringify(qualityData));
                sessionStorage.setItem(cacheKey('recs'), JSON.stringify(recsData));
    ...
    return (
        <div className="p-4 md:p-6 space-y-6 animate-in fade-in duration-500 max-w-[1600px] mx-auto">
            <PrepModal isOpen={isPrepModalOpen} onClose={() => setIsPrepModalOpen(false)} />
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                ...
                <div className="flex items-center gap-2">
                    <Link href="/hr-review">
                        <button
                            className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-500 to-amber-600 text-white rounded-lg text-sm font-medium hover:opacity-90 transition-opacity shadow-sm"
                        >
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 bg-neutral-900 dark:bg-white text-white dark:text-black rounded-lg text-sm font-medium hover:opacity-90 transition-opacity shadow-sm"
                    >
                ...
                <div className="xl:col-span-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <MetricCard
                            ...
                            value={loading ? "..." : `${(isFiniteNumber(summary.deploymentFrequency) ? summary.deploymentFrequency : demoSummary.deploymentFrequency)}/day`}
                            trend={(isFiniteNumber(summary.deploymentFrequency) ? summary.deploymentFrequency : demoSummary.deploymentFrequency) > 3 ? "up" : "down"}
                            trendValue={doraData?.hasData ? "Live" : "Demo"}
                            icon={RocketIcon}
                        />
                        <MetricCard
                            title="Lead Time"
                            value={loading ? "..." : formatNumber(summary.leadTime, "h")}
                            trend={isFiniteNumber(summary.leadTime) ? (summary.leadTime < 4 ? "down" : "up") : "neutral"}
                            trendValue={isFiniteNumber(summary.leadTime) ? (summary.leadTime < 4 ? "Fast" : "Slow") : "N/A"}
                            icon={Zap}
                        />
                        <MetricCard
                            title="Change Failure Rate"
                            value={loading ? "..." : `${(isFiniteNumber(summary.changeFailureRate) ? summary.changeFailureRate : demoSummary.changeFailureRate)}%`}
                            trend={(isFiniteNumber(summary.changeFailureRate) ? summary.changeFailureRate : demoSummary.changeFailureRate) < 10 ? "neutral" : "active"}
                            trendValue={(isFiniteNumber(summary.changeFailureRate) ? summary.changeFailureRate : demoSummary.changeFailureRate) < 10 ? "Good" : "Alert"}
                            icon={ShieldAlert}
                            className={(isFiniteNumber(summary.changeFailureRate) ? summary.changeFailureRate : demoSummary.changeFailureRate) > 15 ? "border-red-500/30 bg-red-500/5" : ""}
                        />
                        <MetricCard
                            title="MTTR"
                            value={loading ? "..." : formatNumber(summary.mttr, "m")}
                            trend={isFiniteNumber(summary.mttr) ? (summary.mttr < 60 ? "down" : "up") : "neutral"}
                            trendValue={isFiniteNumber(summary.mttr) ? (summary.mttr < 60 ? "Excellent" : "Review") : "N/A"}
                            icon={Activity}
                        />
                    </div>
